...
class R2D2Net(torch.jit.ScriptModule):
    ...
    def act(
        ...
        #print("in act: priv_dim",priv_s.shape)
    ...
    def cross_entropy(self, net, lstm_o, target_p, hand_slot_mask, seq_len):
        ...
        print("in ce: ",logit.shape,target_p.shape)
...
class R2D2Agent(torch.jit.ScriptModule):
    ...
    def loss(self, batch, pred_weight, stat):
        ...
        #print("raw shape in r2d2.loss() ",batch.obs["priv_s"].shape)
        #[80, 64, 2, 838]
...
class truncated_R2D2Net(torch.jit.ScriptModule):
    __constants__ = [
        "hid_dim",
        "out_dim",
        "num_lstm_layer",
...
class sad_player2(torch.jit.ScriptModule):
    __constants__ = ["vdn", "multi_step", "gamma", "eta", "uniform_priority"]
    ...
    @torch.jit.script_method
    def get_h0(self, batchsize: int) -> Dict[str, torch.Tensor]:
        return self.net.get_h0(batchsize)
    ...
    def clone(self, device, overwrite=None):
        ...
        cloned = type(self)(
            ...
            self.net.hid_dim,
            self.net.out_dim,
            self.net.num_lstm_layer,
            self.net.hand_size,
            self.uniform_priority,
            num_fc_layer=self.net.num_fc_layer,
            skip_connect=self.net.skip_connect,
        )
        cloned.load_state_dict(self.state_dict())
        return cloned.to(device)
    ...
    def aux_task_iql(self, lstm_o, hand, seq_len, rl_loss_size, stat):
        seq_size, bsize, _ = hand.size()
        own_hand = hand.view(seq_size, bsize, self.net.hand_size, 3)
        own_hand_slot_mask = own_hand.sum(3)
        pred_loss1, avg_xent1, _, _ = self.net.pred_loss_1st(
            lstm_o, own_hand, own_hand_slot_mask, seq_len
        )
        assert pred_loss1.size() == rl_loss_size
    ...
    def aux_task_vdn(self, lstm_o, hand, t, seq_len, rl_loss_size, stat):
        """1st and 2nd order aux task used in VDN"""
        seq_size, bsize, num_player, _ = hand.size()
        own_hand = hand.view(seq_size, bsize, num_player, self.net.hand_size, 3)
        own_hand_slot_mask = own_hand.sum(4)
        pred_loss1, avg_xent1, belief1, _ = self.net.pred_loss_1st(
            lstm_o, own_hand, own_hand_slot_mask, seq_len
        )
        assert pred_loss1.size() == rl_loss_size
...
class R2D2NetNew(torch.jit.ScriptModule):
    __constants__ = [
        "hid_dim",
        "out_dim",
        "num_lstm_layer",
...
class SadPlayer2New(torch.jit.ScriptModule):
    __constants__ = ["vdn", "multi_step", "gamma", "eta", "uniform_priority"]
    ...
    def forward(self, obs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        # print("jit! ",obs.keys())
        if self.vdn:
            obsize, ibsize, num_player = obs["s"].size()[:3]
            priv_s = obs["s"].flatten(0, 2)
            legal_move = obs["legal_move"].flatten(0, 2)
            eps = obs["eps"].flatten(0, 2)
        else:
            obsize, ibsize = obs["s"].size()[:2]
            num_player = 1
            priv_s = obs["s"].flatten(0, 1)
            legal_move = obs["legal_move"].flatten(0, 1)
            eps = obs["eps"].flatten(0, 1)
