...
class R2D2Net(torch.jit.ScriptModule):
    ...
    def forward(
        ...
        #print("in R2D2Net,priv_s shape",priv_s.shape)
        #print("before embed: ",priv_s.shape)
        one_step = False
        if priv_s.dim() == 2:
            priv_s = priv_s.unsqueeze(0)
        ...
        x = self.net(priv_s)
        #print("embed of priv_s: ",x.shape)
        if len(hid) == 0:
            o, (h, c) = self.lstm(x)
        else:
            o, (h, c) = self.lstm(x, (hid["h0"], hid["c0"]))
        #print("lstm: ",o.shape,h.shape,c.shape)
        a = self.fc_a(o)
        v = self.fc_v(o)
        ...
        # print("qa val",qa.shape)
        assert q.size() == legal_move.size()
        #print("q,legal_q",q.shape,legal_q.shape)
        #print("legal_move ",legal_move.shape)
        #print(legal_move[0,0])
        legal_q = (1 + q - q.min()) * legal_move
        #print("q,legal_q",q[0,1])
        #print("legal q",legal_q[0,1])
...
class R2D2Agent(torch.jit.ScriptModule):
    ...
    def act(self, obs: Dict[str, torch.Tensor]) -> Dict[str, torch.Tensor]:
        ...
        """
        #print("now in act!!!!")
        # self.summary_dict(batch.obs)
        # for k,v in obs.items():
        #     print("key: ",str(k))
        #     print("shape: ",v.shape)
        #print("own_hand ",obs["own_hand"][:10,0,:])
        # print(batch.obs["own_hand"][0,0])
    ...
    def td_error(self, obs, hid, action, reward, terminal, bootstrap, seq_len, stat):
        #@wjc test slHand
        #sl_hand=obs["sl_hand"]
        #print("sl_hand shape: ",sl_hand.shape)
        #ow=obs["own_hand"]
        #print("own_hand shape: ",ow.shape)
        max_seq_len = obs["priv_s"].size(0)
        #print("TD_error: priv_s shape:",obs["priv_s"].shape)
    ...
    def aux_task_vdn(self, lstm_o, hand, t, seq_len, rl_loss_size, stat):
        ...
        #print("check own_hand: ",own_hand[1,:5,:,:,:])
        #print("reshaped own hand:",own_hand,own_hand.shape)
    ...
    def loss(self, batch, pred_weight, stat):
        ...
        if pred_weight > 0:
            if self.vdn:
                ...
                #print("priv_s in loss(): ",batch.obs["priv_s"].shape)
                #print("own hand in batch: ",batch.obs["own_hand"],batch.obs["own_hand".shape])
                #print("temperature in batch: ",batch.obs["temperature"],batch.obs["temperature".shape])
                #print("legal move in batch",batch.obs["legal_move"],batch.obs["legal_move".shape])
