...
if __name__ == "__main__":
   ...
   # agent.sync_target_with_online()
    print("game feature / action size: ",games[0].feature_size(),games[0].num_action())
    #loading BP agent
    agent=loadBP("sad-aux-op",args.act_device)
    agent.sync_target_with_online()
    #print("loading BP model: ",agent)
    BeliefEncoder=belief.BeliefEncoder(games[0].feature_size(),1,256)
    BeliefDecoder=belief.BeliefDecoder(256,25,args.train_device)
    BeliefModel=belief.BeliefModel(BeliefEncoder,BeliefDecoder,args.train_device,128)
    BeliefModel=BeliefModel.to(args.train_device)

    root=os.path.dirname(os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
    belief_file=os.path.join(root,"hanabi","pyhanabi", "exps", "lbf","model_epoch300.pthw")
    if not os.path.exists(belief_file):
        print(f"Cannot find weight at: {belief_file}")
        assert False
    device="cuda"
    utils.load_weight(BeliefModel, belief_file, device)
    ...
    #update belief model
    loss = BeliefModel.beliefQuality(batch)#shape [80]
    loss=(loss).detach().numpy()
    pickle.dump( loss, open( "belief_quality.p", "wb" ) )
    print("finish writing to pickle")
            # for batch_id in range(5):
            #     print("batch id: ",batch_id)
            #     for seq_id in range(80):
            #         print(loss[seq_id,batch_id].item())
            # print(loss.mean())
            #print("priority after loss: ", priority.shape)
